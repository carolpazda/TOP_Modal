...
function main(x0::Vector{Float64},verifica_derivada=false)
    ...
    saida2 = "saida_modos.pos"
    Gmsh_init(saida2,mesh)
    ...
    for k=1:NITER
        ...
        Gmsh_element_scalar(mesh,ρ,saida2,"Design variables")

        # Para vermos os modos da estrutura
        for i=1:nmodos
            Gmsh_nodal_vector(mesh,Phi[:,i],saida2,"Modo $i $(omega[i]/(2*pi))[Hz]")
        end
